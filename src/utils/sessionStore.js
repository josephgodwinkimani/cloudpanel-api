...
class SessionStore {
  ...
  async initialize() {
    try {
      ...
      logger.warn('Failed to initialize Redis store, falling back to SQLite store:', error.message);
      this.initializeSQLiteStore();
    }
  }
  ...
  initializeSQLiteStore() {
    ...
    logger.info(`Session database location: ${path.join(sessionsDir, 'sessions.db')}`);
  }
  ...
  initializeMemoryStore() {
    ...
    if (process.env.NODE_ENV === 'production') {
      logger.warn('WARNING: Using MemoryStore in production is not recommended. Consider setting up Redis or use SQLite (default).');
      logger.warn('To use Redis, set REDIS_URL or REDIS_HOST environment variable.');
    } else {
